// Validation zod cho 3 API quản lý tài khoản của admin (list/detail/khóa-mở khóa).
import { z } from "zod";
...
/** Validate query của GET /admin/users. */
...
const statusBodySchema = z
  ...
  .refine((data) => data.status !== "BANNED" || !!data.reason, {
    error: "Lý do là bắt buộc khi khóa tài khoản",
    path: ["reason"],
  });

export type StatusBody = z.infer<typeof statusBodySchema>;

/** Validate body của PUT /admin/users/{id}/status — reason bắt buộc khi khóa. */
