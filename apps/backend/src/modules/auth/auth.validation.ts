...
const publicRoleSchema = z.enum([ROLES.CANDIDATE, ROLES.RECRUITER], {
  error : "role chỉ nhận CANDIDATE hoặc RECRUITER" });

export const registerSchema = z.object({
  email: emailSchema,
  password: passwordSchema,
  fullName: z
    .string({ error: "fullName là bắt buộc" })
    .trim()
...
export const resetPasswordSchema = z.object({
  resetToken: z
    .string({ error: "resetToken là bắt buộc" })
    .trim()
...
export const googleLoginSchema = z.object({
  idToken: z
    .string({ error: "idToken là bắt buộc" })
    .trim()
