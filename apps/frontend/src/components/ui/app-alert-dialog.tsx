...
const toneStyles: Record<AlertTone, { icon: typeof AlertCircle; media: string; confirm: string }> = {
  error: {
    icon: TriangleAlert,
    media: "bg-danger/10 text-danger",
  ...
  success: {
    icon: CheckCircle2,
    media: "bg-success/10 text-success",
  ...
  return (
    <AlertDialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <AlertDialogPrimitive.Portal>
        ...
        <AlertDialogPrimitive.Popup className="fixed left-1/2 top-1/2 z-50 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-xl border border-slate-200 bg-white text-text shadow-[0_24px_70px_rgba(15,23,42,0.24)] outline-none transition duration-150 data-closed:scale-95 data-closed:opacity-0 data-open:scale-100 data-open:opacity-100">
          <header className="flex items-center gap-3 px-6 py-5">
            <span className={cn("flex size-10 shrink-0 items-center justify-center rounded-full", style.media)}>
              <Icon className="size-5" aria-hidden="true" />
            </span>
            <AlertDialogPrimitive.Title className="min-w-0 flex-1 text-lg font-semibold leading-6 text-text">
              {title}
          ...
          <AlertDialogPrimitive.Description className="border-y border-slate-200 px-6 py-8 text-sm leading-6 text-muted">
            {description}
          </AlertDialogPrimitive.Description>

          <footer className="flex flex-col-reverse gap-3 px-6 py-6 sm:flex-row sm:justify-end">
            {showCancel ? (
              <AlertDialogPrimitive.Close
                disabled={isConfirming}
                render={<Button type="button" variant="outline" className="h-10 min-w-20 border-slate-300 bg-white px-5 text-text hover:bg-slate-50" />}
              >
                {cancelLabel}
              </AlertDialogPrimitive.Close>
            ) : null}
