    ...
    <section className="overflow-hidden rounded-lg border border-border bg-surface shadow-sm">
      ...
      {tab === "DESCRIPTION" && (
        <div className="space-y-6 p-5 text-xs leading-6 text-muted">
          <section>
            <h2 className="mb-2 text-sm font-semibold text-text">Mô tả công việc</h2>
            <div className="prose prose-xs max-w-none text-xs leading-6 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5" dangerouslySetInnerHTML={{ __html: job.description }} />
          </section>
          <section>
            <h2 className="mb-2 text-sm font-semibold text-text">Yêu cầu ứng viên</h2>
            <div className="prose prose-xs max-w-none text-xs leading-6 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5" dangerouslySetInnerHTML={{ __html: job.requirements }} />
          </section>
          {job.benefits && (
            <section>
              <h2 className="mb-2 text-sm font-semibold text-text">Quyền lợi</h2>
              <div className="prose prose-xs max-w-none text-xs leading-6 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5" dangerouslySetInnerHTML={{ __html: job.benefits }} />
            </section>
          )}
