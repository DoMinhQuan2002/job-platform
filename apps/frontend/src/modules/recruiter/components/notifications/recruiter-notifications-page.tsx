...
import { NotificationListItem } from "@/modules/notifications/components/notification-list-item";
import { RECRUITER_NOTIFICATION_TABS } from "@/modules/notifications/lib/format";
...
export function RecruiterNotificationsPage() {
  ...
  const activeTypes = useMemo<NotificationType[] | undefined>(
    () => RECRUITER_NOTIFICATION_TABS.find((tab) => tab.value === activeTab)?.types,
    [activeTab],
  );
  ...
  const loadTabCounts = useCallback(async () => {
    try {
      const results = await Promise.all(
        RECRUITER_NOTIFICATION_TABS.map((tab) =>
          notificationsApi.list({ page: 1, limit: 1, type: tab.types }),
        ),
      );
      const counts: Record<string, number> = {};
      RECRUITER_NOTIFICATION_TABS.forEach((tab, index) => {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
        <header className="space-y-1.5">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Thông báo tuyển dụng</h1>
          <p className="max-w-xl text-sm text-muted">
            Theo dõi các ứng viên mới, cập nhật tin tuyển dụng và trạng thái doanh nghiệp từ hệ thống.
          </p>
        </header>
        ...
        <div className="space-y-5 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-xs sm:p-8">
          ...
          {loading ? (
            ...
            <div className="space-y-3 rounded-2xl border border-rose-100 bg-rose-50 p-6 text-sm text-rose-800">
              ...
              {unauthorized ? (
                <Link
                  href={`${ROUTES.auth.login}?redirect=${encodeURIComponent(ROUTES.recruiter.notifications)}`}
            ...
            <div className="py-12 text-center">
              ...
              <p className="mx-auto mt-1 max-w-md text-xs text-slate-500 sm:text-sm">
                Thông báo mới sẽ xuất hiện tại đây khi có ứng viên mới hoặc biến động tin đăng.
              </p>
            </div>
          ) : (
