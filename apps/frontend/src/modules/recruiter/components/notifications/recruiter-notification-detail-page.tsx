...
import type { NotificationDetail } from "@/modules/notifications/types";
...
export function RecruiterNotificationDetailPage() {
  ...
  const handleDelete = async () => {
    try {
      ...
      router.push(ROUTES.recruiter.notifications);
    } catch (err) {
  ...
  if (error || !notification) {
    return (
      <div className="space-y-3 rounded-3xl border border-rose-100 bg-rose-50 p-8 text-sm text-rose-800 shadow-xs">
        ...
        {unauthorized ? (
          <Link
            href={`${ROUTES.auth.login}?redirect=${encodeURIComponent(`${ROUTES.recruiter.notifications}/${id}`)}`}
    ...
    <div className="mx-auto max-w-5xl space-y-6">
        <header className="space-y-1.5">
          <nav className="flex items-center gap-1.5 text-xs text-muted">
            ...
            <Link href={ROUTES.recruiter.notifications} className="hover:text-primary">
              Thông báo
            </Link>
            <ChevronRight className="size-3 text-muted" />
          ...
          <div className="space-y-5 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-xs sm:p-8">
            ...
            {job ? (
              <div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50/60 p-5">
                <h3 className="text-sm font-bold text-slate-900">Thông tin tin tuyển dụng</h3>
                <div className="flex items-start gap-3">
                  <CompanyLogo
                    name={job.company?.name ?? "Doanh nghiệp"}
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  ...
                  <Link
                    href={ROUTES.recruiter.candidates}
                    className="inline-flex rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50"
                  >
                    Quản lý ứng viên
                  </Link>
                </div>
            ...
            <div className="rounded-2xl border border-blue-100 bg-blue-50/80 p-4 text-xs text-blue-900">
              Đây là thông báo tự động từ hệ thống quản lý tuyển dụng.
            </div>
          </div>
