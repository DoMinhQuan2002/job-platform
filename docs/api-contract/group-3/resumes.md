...
| `GET /api/resumes/{id}` | 1 row + ownership | *(chưa có trong stub)* | Có |
...
*(Alternative: `POST /resumes` nhận multipart và gọi storage nội bộ — cần Leader chọn 1 flow để FE khỏi lệch.)*
...
| Quyền | `CANDIDATE` (owner) — *Recruiter xem snapshot qua application, không qua API này (chốt thêm nếu cần)* |
